...
public class GameScreen extends Screen {
	...
	public GameScreen(Game game) {
		...
		// Transfering images from assets to this class
		...
		// Defining animations of character and enemies
		anim = new Animation();
	...
	private void loadMap() {
		...
		while (scanner.hasNextLine()) {
			...
			if (!line.startsWith("!")) {
				lines.add(line);
				// width keeps increasing and eventually reaches its max size
		...
		// TODO: 12 gibi hard number kullanmak yerine bi sekilde line sayisini
		// al
		// while icinde counter? gerci sanirim height ile bunu yapabiliriz zaten
		// ! leri geciyoz cunku
		for (int i = 0; i < 12; i++) {
			String line = (String) lines.get(i);
			for (int j = 0; j < width; j++) {
				// TODO: else ile continue yaptirip width e gelmeden bosa
				// donmesini engelleyebiliriz?
				if (j < line.length()) {
	...
	public void update(float deltaTime) {
		...
		// Update method works like in the example. One update method calls four
		// other methods
	...
	private void updateReady(List touchEvents) {
		// First update screen
		// Changes GameState when user touches the screen
	...
	private void updateRunning(List touchEvents, float deltaTime) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (event.type == TouchEvent.TOUCH_DOWN) {
				// TODO: Instead of calling a bunch of inBounds, we can enum
				// robot states?
